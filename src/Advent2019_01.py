...
#
masses = list(map(int, [mass.strip() for mass in open('../inputs2019/Advent2019_01.txt', 'r')]))
fuel = lambda x: x // 3 - 2
...
print(f'#1: {sum(map((fuel := lambda x: x // 3 - 2), (masses := list(map(int, [mass.strip() for mass in open("../inputs2019/Advent2019_01.txt", "r")])))))}\n#2: {sum(map((fuel_recursive := lambda x: x > 0 and (y := fuel(x)) + max(fuel_recursive(y), 0)), masses))}')
