...
combos <- as_tibble(gtools::combinations(n = 200, r = 2, v = inp)) %>%
  rowwise() %>%
  filter(sum(c(V1, V2)) == 2020) %>%
  mutate(product = V1 * V2)

print(stringr::str_interp("AoC 2020 Day 1, Part 1 answer is (${V1}, ${V2}) is ${product}", combos[1,]))


combos <- as_tibble(gtools::combinations(n = 200, r = 3, v = inp)) %>%
  rowwise() %>%
  filter(sum(c(V1, V2, V3)) == 2020) %>%
